...
import { authenticate, requireRole, signToken, } from "./middleware/authContext.js";
import { userRepository, accountRepository, eventRepository, auctionRepository, bankOfferRepository } from "./repositories/repositories.js";
import { AUCTION_DURATION_DAYS } from "./models/types.js";
import { CrudService } from "./services/crudService.js";
import { isExpired } from "./services/auctionRules.js";
import { canManageAccount, scopeAccountsForUser, scopeEventsForUser, stripAccountPII, toPublicUser, } from "./services/rbac.js";
...
const accountService = new CrudService(accountRepository);
const eventService = new CrudService(eventRepository);
const auctionService = new CrudService(auctionRepository);
const bankOfferService = new CrudService(bankOfferRepository);
/** All offers submitted for a given auction. */
async function offersForAuction(auctionId) {
...
async function settleAndPersist(auction, options = {}) {
...
const VALID_ROLES = ["ADMIN", "MANAGER", "USER", "BANKER"];
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
app.get("/accounts", requireRole("ADMIN", "MANAGER", "USER", "BANKER"), async (req, res) => {
    const user = req.user;
    const scoped = scopeAccountsForUser(await accountService.findAll(), user, await eventService.findAll(), await auctionService.findAll());
    const result = user.role === "BANKER" ? scoped.map(stripAccountPII) : scoped;
    res.json(result);
});
app.get("/accounts/:id", requireRole("ADMIN", "MANAGER", "USER", "BANKER"), async (req, res) => {
    const user = req.user;
    ...
    const visible = scopeAccountsForUser([account], user, await eventService.findAll(), await auctionService.findAll());
    if (visible.length === 0) {
        return res.status(404).json({ message: "Not found" });
    }
...
app.post("/accounts", requireRole("ADMIN", "MANAGER"), async (req, res) => {
    const user = req.user;
...
// ---------------------------------------------------------------------------
app.get("/events", requireRole("ADMIN", "MANAGER", "USER"), async (req, res) => {
    const user = req.user;
    const scoped = scopeEventsForUser(await eventService.findAll(), user, await accountService.findAll());
    res.json(scoped);
});
/** Create an event. createdByUserId is always the authenticated user. */
app.post("/events", requireRole("ADMIN", "MANAGER", "USER"), async (req, res) => {
    const user = req.user;
    try {
 ...
 */
app.get("/auction-opportunities", requireRole("ADMIN", "MANAGER", "BANKER"), async (req, res) => {
 ...
 */
app.post("/auction-opportunities", requireRole("ADMIN", "MANAGER"), async (req, res) => {
    const user = req.user;
 ...
 */
app.post("/auction-opportunities/:id/close", requireRole("ADMIN", "MANAGER"), async (req, res) => {
    const id = req.params.id;
    let auction;
    try {
 ...
 */
app.post("/bank-offers", requireRole("ADMIN", "BANKER"), async (req, res) => {
    const currentUser = req.user;
    ...
    const bankId = currentUser.role === "BANKER" ? currentUser.bankId : req.body.bankId;
    if (!bankId) {
    ...
    const alreadySubmitted = (await offersForAuction(auctionId)).some((o) => o.bankId === bankId);
    if (alreadySubmitted) {
        return res.status(409).json({
            message: "This bank has already submitted an offer for this auction (sealed auction)",
        });
    }
...
app.get("/bank-offers", requireRole("ADMIN", "BANKER"), async (req, res) => {
    const currentUser = req.user;
...
app.post("/accounts/:id/auctions", requireRole("ADMIN", "MANAGER"), async (req, res) => {
    const user = req.user;
...
app.post("/auctions/:id/offers", requireRole("ADMIN", "BANKER"), async (req, res) => {
    const currentUser = req.user;
...
app.post("/auctions/:id/close", requireRole("ADMIN", "MANAGER"), async (req, res) => {
    const id = req.params.id;
    let auction;
    try {
        auction = await auctionService.findById(id);
...
app.get("/analytics/summary", requireRole("ADMIN", "MANAGER"), async (req, res) => {
    const user = req.user;
