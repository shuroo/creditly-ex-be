...
beforeEach(() => {
    deps = {
        ...
        settle: vi.fn().mockImplementation(async (auction) => auction),
    };
});
describe("runExpirySweep", () => {
    ...
    it("calls settle for each auction", async () => {
        ...
        deps.listAuctions.mockResolvedValue(auctions);
    ...
    it("counts auctions whose status changed after settle", async () => {
        ...
        deps.listAuctions.mockResolvedValue([auction]);
        deps.settle.mockImplementation(async (a) => {
            a.status = "EXPIRED";
            return a;
        });
...
describe("3-day expiry — end-to-end via real settleAuction", () => {
    ...
    function realSettle(offers) {
        return async (auction) => {
...
//# sourceMappingURL=auctionExpiryService.test.js.map